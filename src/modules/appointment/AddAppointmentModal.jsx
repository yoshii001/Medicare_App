...
import { getPatients, getDoctors, getClinics, getTimeSlots } from '../../services/database';
...
const AddAppointmentModal = ({ onClose, onSave }) => {
  ...
  const handleClinicSelect = async (e) => {
    ...
    const slotsData = await getTimeSlots(selectedClinic.id);
    if (slotsData) {
      const availableSlots = Object.values(slotsData).filter(slot => slot.status === 'available');
...
const AddAppointmentModal = ({ onClose, onSave }) => {
  ...
  const handleClinicSelect = async (e) => {
    ...
    const slotsData = await getTimeSlots(selectedClinic.id);
    if (slotsData) {
      const availableSlots = Object.values(slotsData).filter(
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end space-x-2">
            <button
              type="button"
              onClick={onClose}
              className="btn-secondary bg-gray-200 px-4 py-2 rounded"
            >
              Cancel
            </button>
            <button
